...
function prefill_tasks(project_tasks, project) {
   const p_store = JSON.parse(localStorage.getItem(project.Name));
   if (p_store != null) {
      p_store.forEach(element => {
         const local_task = Task(element.title, document.createElement('button'), element.date);
         Fill_tasks(project_tasks, local_task);
         local_task.Del.addEventListener('click', () => {
            let i = project.Storage.findIndex((item) => item.title === local_task.title);
            console.log(project.Storage);
            if(i!=-1) project.Storage.splice(i, 1);
            console.log(i);
            console.log(project.Storage);
            localStorage.setItem(project.Name,JSON.stringify(project.Storage));
...
function project_list(project) {
   function Task_form() {
      ...
      task_form.addEventListener('submit', function (event) {
         ...
         const dummy_task = Task(task_title, document.createElement('button'), JSON.stringify(task_date));
         project.Storage.push(dummy_task);
         localStorage.setItem(project.Name, JSON.stringify(project.Storage));
         console.log(JSON.parse(localStorage.getItem(project.Name)));
         dummy_task.Del.addEventListener('click', () => {
            let i = project.Storage.findIndex((item) => item.title === dummy_task.title);
            if(i!=-1) project.Storage.splice(i, 1);
            console.log(project.Storage);
            localStorage.setItem(project.Name,JSON.stringify(project.Storage));
            console.log(JSON.parse(localStorage.getItem(project.Name)));
   ...
   prefill_tasks(project_tasks, project);
...
function Tasks() {
   ...
   task_list.appendChild(project_list(p_Array[0]));
